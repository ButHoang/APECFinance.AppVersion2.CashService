...
public interface InvestorCashTransactionRepository extends JpaRepository<InvestorCashTransactionEntity, Long> {
    ...
    @Query("SELECT t FROM InvestorCashTransactionEntity t WHERE t.tranDate >= :startDate AND t.tranDate <= :endDate AND t.tranType IN :tranTypes AND t.investorId = :investorId")
